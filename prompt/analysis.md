...
Memory safety reports are the crashes that the fuzzer is looking for.
...
If you cannot decide on what crash you are looking at, treat it as a memory-safety report.
A missed round is expensive and a constraint that quietly suppresses the finding costs the whole experiment.

Report it. The finding is this round's product and it is already recorded, in
this round's crash directory and in the round's commit. Then do both of the
following in the same round. Neither weakens the report, and leaving the tree
untouched reproduces this round exactly — same seeds, same binary, same crash,
for every round after it.

1. **Retire the seed that reaches it, and replace it.** Under `panic_on_warn`
   the report kills the VM, and there is exactly one VM, so a seed that reaches
   this crash during triage takes it again on every fresh VM and the next round
   spends itself rediscovering what this one found. Delete it (`seeds`,
   `action: delete`; git keeps the file) and write one that exercises the
   deepened primitive from (2) instead. If no seed reaches it — the fuzzer got
   there by mutation — say so and do (2) only.

2. **Deepen the primitive that reached it**, so the next round explores past
   this crash instead of back into it. Widen an argument the harness currently
   pins, or type as a syzlang struct what is currently an opaque byte array, so
   the fuzzer mutates fields rather than replaying a page. Cite the line in the
   report or in the source that identifies which argument is the bottleneck.
   This is a change to `constraints` like any other: fill in `forfeit`.
...
becomes exactly the accumulated hypothesis this design removes.
